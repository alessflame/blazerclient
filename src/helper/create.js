...
export const createPost = async (form, user) => {
  ...
  const response = await axios.post("https://blazerstravels.onrender.com/posts", formData ,{
    headers: {
...
export const createComment = async(form, id_user, id_post)  => {
  ...
  const response =await axios.post(`https://blazerstravels.onrender.com/comments/${id_post}`, commentJson, {
    headers: {
...
export const createTravel=async(form, user, countriesSelected)=>{
    ...
    //  console.log(formData.get("countries"));
     ...
     const response = await axios.post("https://blazerstravels.onrender.com/travels", formData ,{
      headers: {
        "Authorization": getToken()
      },
    });
